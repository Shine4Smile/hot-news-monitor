...
async function callDeepSeek(messages: { role: string; content: string }[], options: { temperature?: number; maxTokens?: number } = {}): Promise<string> {
...
4. 评估热点的重要程度（importance），对关注"${keyword}"的用户来说：
   - urgent：突发/重大新闻，行业地震级事件，官方重大公告，安全漏洞预警，必须在第一时间知晓的内容
   - high：重要产品发布/更新，深度分析/评测，知名人物观点，有实质信息量的内容
   - medium：有一定参考价值的相关资讯、讨论、教程、经验分享
   - low：仅简单提及关键词，或内容价值较低（如纯转发、一句话带过、SEO 水文）
